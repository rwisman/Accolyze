 ...
 * 			March 3, 2014
 * 
 * 	Accolyze - Accelerometer Collect and Analyze.  
...
public class AccolyzeActivity extends Activity {
	...
	private static final String DIRECTORY = "accData";
	private static final String PROGRAM = "Accelerometer Monitor";
	private static final String PACKAGENAME="com.lul.accelerometer";
	...
	private void analyze() {
	    ...
	    if (!action.equals(Intent.ACTION_VIEW)) {						// App started directly, try to find Accelerometer Monitor data directory and most recent file.	    	
	    	try {
		...
		try {
			String filepath = Environment.getExternalStorageDirectory().getPath();
			xlsDir = new File(filepath, "Accolyze");		
			...
			for( int i=0; i<8; i++) br.readLine();				// First 8 lines of data are comments
			...
			while((line=br.readLine()) != null) {
				...
				if(values.length == 4){
					...
					for(int col=1; col<5; col++) {
						d = Double.parseDouble(values[col-1]);
